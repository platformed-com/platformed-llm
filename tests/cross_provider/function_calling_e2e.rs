...
use super::providers::{ProviderTestSetup, create_weather_tool, openai::OpenAITestSetup, google::GoogleTestSetup, anthropic::AnthropicTestSetup};
...
async fn run_function_calling_test<T: ProviderTestSetup>() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !config.supports_custom_base_url {
        println!("⚠️  {} provider doesn't support custom base URLs for mocking", config.name);
    ...
    let mut conversation = Prompt::system("You have access to weather data. Use the get_weather function when asked about weather.")
    ...
    let response = provider.generate(&request).await.expect("Failed to get response from provider");
    ...
    while let Some(event_result) = stream.next().await {
        ...
        accumulator.process_event(event).expect("Event processing should succeed");
    }
    
    // ASSERTION 2: Verify we got the expected function call from parsing
    let function_calls = accumulator.completed_function_calls();
    assert!(!function_calls.is_empty(), "{}: Should have at least one function call", config.name);
    
    let weather_call = &function_calls[0];
    assert_eq!(weather_call.name, "get_weather", "{}: Function name should be get_weather", config.name);
    assert!(weather_call.arguments.contains("Paris"), "{}: Function arguments should contain Paris", config.name);
    ...
    if config.name == "OpenAI" {
        assert_eq!(weather_call.id, "call_abc123def456", "OpenAI: Function call ID should match fixture");
    }
    ...
    conversation = conversation.with_item(InputItem::function_call_output(weather_call.call_id.clone(), function_result.to_string()));
    ...
    let followup_response = provider.generate(&followup_request).await.expect("Failed to get follow-up response");
    ...
    assert!(!followup_text.trim().is_empty(), "{}: Follow-up response should not be empty", config.name);
    ...
    println!("\n✅ {} End-to-end function calling test PASSED with comprehensive assertions!", config.name);
