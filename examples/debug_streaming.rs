...
async fn main() -> Result<(), Error> {
    ...
    // Wrap the entire generate-and-stream-consumption in `retry` so a
    // transient 429 / 5xx / mid-stream drop restarts cleanly. The
    // closure is the unit of retry: any state it owns (the event
    // counter, the accumulator) is discarded on each attempt, so a
    // partial stream from a previous attempt can't leak into the
