...
async fn main() -> Result<(), Error> {
    ...
    println!("✅ Google (with ADC): {:?} in {:?}", 
        google_convenience.provider_type, 
        google_convenience.location
    );
    println!("✅ Anthropic (with token): {:?} in {:?}", 
        anthropic_convenience.provider_type, 
        anthropic_convenience.location
    );
    ...
    println!("✅ Access token: provider={:?}, has_token={}", 
        with_token.provider_type, with_token.access_token.is_some());
    println!("✅ ADC: provider={:?}, has_token={}", 
        with_adc.provider_type, with_adc.access_token.is_some());
