...
use crate::{Error, Usage, FinishReason, FunctionCall, StreamEvent};
...
mod tests {
    ...
    fn test_complete_response_creation() {
        let response = CompleteResponse {
            output: vec![OutputItem::Text { content: "Hello, world!".to_string() }],
    ...
    async fn test_response_buffering() {
        ...
        let events = vec![
            ...
            Ok(StreamEvent::ContentDelta { delta: "Test response".to_string() }),
            Ok(StreamEvent::Done { 
                finish_reason: FinishReason::Stop, 
                usage: Usage::default() 
            }),
    ...
    fn test_mixed_output_ordering() {
        ...
        let response = CompleteResponse {
            output: vec![
                OutputItem::Text { content: "I'll help you with that. ".to_string() },
                ...
                OutputItem::Text { content: " Let me also check something else.".to_string() },
            ],
            finish_reason: FinishReason::Stop,
            usage: Usage::default(),
        };
        ...
        assert_eq!(response.content(), "I'll help you with that.  Let me also check something else.");
        ...
        match &response.output[2] {
            OutputItem::Text { content } => assert_eq!(content, " Let me also check something else."),
            _ => panic!("Expected text item"),
        }
    }
    ...
    fn test_to_items_mixed() {
        ...
        let mixed_response = CompleteResponse {
            output: vec![
                ...
                OutputItem::Text { content: "Let me check that for you.".to_string() },
            ],
            finish_reason: FinishReason::ToolCalls,
            usage: Usage::default(),
        };
        ...
        match &items[2] {
            crate::types::InputItem::Message(msg) => {
                assert_eq!(msg.role(), Role::Assistant);
                assert_eq!(msg.content(), Some("Let me check that for you.".to_string()));
            }
            _ => panic!("Expected message"),
        }
    }
